...
echo "Built frame.mcp plugin -> plugin.wasm"
...
if command -v wasm-tools &>/dev/null && command -v python3 &>/dev/null; then
	wasm-tools print plugin.wasm > /tmp/frame_mcp_strip.wat
	python3 ../../scripts/strip-unused-wasm-imports.py /tmp/frame_mcp_strip.wat /tmp/frame_mcp_stripped.wat
	wasm-tools parse /tmp/frame_mcp_stripped.wat -o plugin.wasm
	rm -f /tmp/frame_mcp_strip.wat /tmp/frame_mcp_stripped.wat
