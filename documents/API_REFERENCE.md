...
> 🚧 **IN ACTIVE DEVELOPMENT — Phase 2 underway.** This section describes the v2 data model (paired with plugin spec `frame-data.ebnf` v2.0.0), the target for the next `frame.data` release. **The currently shipped plugin is `frame.data` v3.0.10** and does not yet accept `Database.save(entity)`, `Entity.data.method()`, or sub-block `data <T>:`. For code you write today, use v1.2 syntax (`Model.insert:`, `Model.update:`, `Model.delete:`, `Model.upsert:`, bare-field `data:` declarations). When Phase 2 sub-cycles complete, this banner will be removed as the plugin catches up to the spec. See [04_frame_data.md](specification/04_frame_data.md) header for full context.
