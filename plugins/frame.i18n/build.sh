...
echo "Built frame.i18n plugin -> plugin.wasm"
...
if command -v wasm-tools &>/dev/null && command -v python3 &>/dev/null; then
	wasm-tools print plugin.wasm > /tmp/frame_i18n_strip.wat
	python3 ../../scripts/strip-unused-wasm-imports.py /tmp/frame_i18n_strip.wat /tmp/frame_i18n_stripped.wat
	wasm-tools parse /tmp/frame_i18n_stripped.wat -o plugin.wasm
	rm -f /tmp/frame_i18n_strip.wat /tmp/frame_i18n_stripped.wat
